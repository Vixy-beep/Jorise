...
class RiskCase(models.Model):
    """Unified risk case generated by fusion engine for mobile/process scenarios."""
    STATUS_CHOICES = [
        ('open', 'Open'),
        ('triage', 'Triage'),
        ('contained', 'Contained'),
    ...
    organization = models.ForeignKey(
        Organization, on_delete=models.CASCADE, null=True, blank=True, related_name='risk_cases'
    )

    source_channel = models.CharField(max_length=20, choices=SOURCE_CHOICES, default='mobile')
    process_name = models.CharField(max_length=100, default='mobile_access')
    attack_pattern = models.CharField(max_length=120, blank=True)
    category = models.CharField(max_length=120, blank=True)
    asset_criticality = models.CharField(
        max_length=20, choices=ASSET_CRITICALITY_CHOICES, default='medium'
    )
    ...
    control_strength_score = models.FloatField(
    ...
    confidence = models.FloatField(
        validators=[MinValueValidator(0), MaxValueValidator(1)], default=0.0
    )

    model_score = models.FloatField(
        validators=[MinValueValidator(0), MaxValueValidator(100)], default=0
    )
    rules_score = models.FloatField(
        validators=[MinValueValidator(0), MaxValueValidator(100)], default=0
    )
    anomaly_score = models.FloatField(
        validators=[MinValueValidator(0), MaxValueValidator(100)], default=0
    )
    sequence_score = models.FloatField(
        validators=[MinValueValidator(0), MaxValueValidator(100)], default=0
    )
    ...
    def __str__(self):
        return f"{self.process_name} [{self.residual_risk_score:.1f}] {self.category}"


class RiskEvidence(models.Model):
    """Evidence attached to a RiskCase for auditability and replay."""
    EVIDENCE_TYPES = [
        ('input', 'Input'),
        ('features', 'Features'),
        ('scoring', 'Scoring'),
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    risk_case = models.ForeignKey(RiskCase, on_delete=models.CASCADE, related_name='evidences')
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['risk_case', 'evidence_type']),
        ]

    def __str__(self):
        return f"{self.risk_case_id} - {self.evidence_type}"


class RiskEngineVersionLog(models.Model):
    """Versioned registry for rule and model bundles used by risk engine."""
    ENGINE_TYPE_CHOICES = [
        ('rules', 'Rules'),
        ('model', 'Model'),
        ('fusion', 'Fusion'),
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    engine_type = models.CharField(max_length=20, choices=ENGINE_TYPE_CHOICES)
    ...
    def __str__(self):
        return f"{self.engine_type}:{self.version}{' (active)' if self.is_active else ''}"
